...
def main():
    ...
    tmp_file_name = os.environ.get(
        'NZBOP_TEMPDIR') + '/PasswordDetector/' + os.environ.get(Prefix + 'NZBID')
    ...
    if os.environ.get('NZBNA_EVENT') == 'NZB_ADDED' or \
        ...
        if not os.environ.get('NZBPR_FAKEDETECTOR_SORTED') == 'yes':
            print(
                '[INFO] Sorting inner files for earlier fake detection for %s' % NzbName)
            sys.stdout.flush()
            sort_inner_files()
