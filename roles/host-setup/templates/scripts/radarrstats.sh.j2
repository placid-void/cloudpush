...
DB="/opt/appdata/radarr/nzbdrone.db"
...
QUERY="SELECT COUNT(*) FROM Movies WHERE id IN (SELECT MovieId FROM MovieFiles WHERE Quality LIKE '%: 1,%' )"
RESULT=$(sqlite3 -header -line "$DB" "$QUERY")
echo "${RESULT:11}   SDTV movies"
